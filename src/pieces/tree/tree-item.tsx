...
function Preview({ item }: { item: TreeItemType }) {
  return <div className={styles.preview}>Item {item.id}</div>;
}
          ...
          >
            <span
              className={[
                styles.innerButton,
                state === "dragging"
                  ? styles.innerDragging
                  : state === "parent-of-instruction"
                  ? styles.parentOfInstruction
                  : undefined,
              ]
                .filter(Boolean)
                .join(" ")}
            >
              <Icon item={item} />
              <span className={styles.label}>Item {item.id}</span>
              <small className={styles.id}>
                {item.isDraft ? <code>Draft</code> : null}
                <code className={styles.debug}>({mode})</code>
              </small>
            </span>
